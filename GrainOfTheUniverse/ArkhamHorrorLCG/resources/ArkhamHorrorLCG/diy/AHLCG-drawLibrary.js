...
//		Enemy orientation (Weaver), collection info on side - this is not happening...

// horizontal guide box spacing

//test treachery weakness subtype

//test per investigator symbols
//test unique star
...
function drawTemplate( g, sheet, className ) {
	...
	if ( useReplacementTemplate( g, sheet, faceIndex )) return;
...
function useReplacementTemplate(g, sheet, faceIndex ) {	
...
function drawName( g, diy, sheet, nameBox ) {
	...
	var unique = '';
/*
//var rh = new RenderingHints(RenderingHints.KEY_TEXT_ANTIALIASING, RenderingHints.VALUE_TEXT_ANTIAIAS_ON);
...
*/
//var desktopHints = Toolkit.getDefaultToolkit().getDesktopProperty("awt.font.desktophints");
	...
	if ( title.length() >  0) {
		...
		if ( CardTypes[faceIndex] == 'Asset' ) {
			...
			if ( classCount >= 2 ) {
				...
				if ( classCount > 2 ) region.width -= 60;	// don't move the x value here though, it stays offcenter
...
function drawIndentedStoryBody( g, diy, sheet, traitsBox, headerBox, storyBox, bodyBox ) {
	...
	var horizLineSpace1 = 4;
	var horizLineSpace2 = 12;
//	var horizLineSpace2 = 14;
...
*/
	...
	horizLineSpace1 += offset;
	horizLineSpace2 -= offset;
...
//	let style = bodyBox.getDefaultStyle();
	var bodyDefaultSize = bodyBox.defaultStyle.get( SIZE );
//	style = traitsBox.getDefaultStyle();
//	style = headerBox.getDefaultStyle();
	var headerDefaultSize = headerBox.defaultStyle.get( SIZE );
//	style = storyBox.getDefaultStyle();
...
//?	totalHeight -= 28;	// don't need the final rule/spacing, tweaked to make Victory look better
	...
	if (totalHeight > fullHeight) {
		textScale = Math.sqrt( scale ) * 0.93;
	}
...
//					new Region( headerRegion.x, headerRegion.y + ( horizLineSpace1 * scale * $ScaleModifier / 100.0 ), headerRegion.width, 7) );
				...
				headerRegion.y += Math.ceil( horizLineSpace2 );
			}
		}
...
//				new Region( storyRegion.x - 18, storyRegion.y, 6, storyRegion.height - 2) );
			g.setPaint( new Color( 0.0, 0.0, 0.0 ) );
			g.setStroke( new BasicStroke( 1.0 ) );
			g.drawLine(storyRegion.x - 15, storyRegion.y + 1, storyRegion.x - 15, storyRegion.y + storyRegion.height - 7);
			g.drawLine(storyRegion.x - 12, storyRegion.y + 1, storyRegion.x - 12, storyRegion.y + storyRegion.height - 7);
		}
...
function drawVictory( g, diy, sheet, victoryBox ) {
	...
	victoryBox.markupText = '<vic>' + $( 'Victory' + BindingSuffixes[faceIndex] ) + '</vic>';
...
//	if ( Eons.namedObjects.AHLCGObject.bodyFamily == 'Times New Roman' ) region.y -= 2;
	...
	if ( height < 15 ) {	// fits on one line
		...
		text = $ScenarioIndex + '<sufb>' + String.fromCharCode( $ScenarioDeckID.charCodeAt(0) + 1 ) + '</sufb>';
